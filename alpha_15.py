...
def main():
	...
	generate_lcg(numbers, key, serial_number, batch_number)
...
def generate_lcg(num_iterations, key, serial_number, batch_number):
   ...
   """
	...
	while counter < num_iterations:
		...
		x_value_10 = str(x_value).zfill(10)
		plaintext_bytes = [ord(i) for i in x_value_10]
		a_sum = 0
		#sum = 0
		# for value in plaintext_bytes:
		# 	sum = sum + value
		a_sum = sum(plaintext_bytes)
		sec = str(a_sum).zfill(3)

		if int(serial_number) <= 9:
			number = serial_number[1] + batch_number + sec[2] + x_value_10 + serial_number[0]
		else:
			number = serial_number + batch_number + sec[2] + x_value_10
